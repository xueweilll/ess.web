...
public class DTUAction extends ActionBase {
	...
	public String vedioDtu(){
		dtu = dtuService.dtu(dtuId);
		map = new HashMap<>();
		map.put("id", dtu.getId());
		map.put("ip", dtu.getNvrIp());
		map.put("username", dtu.getNvrUsername());
		map.put("password", dtu.getNvrPassword());
		map.put("path", dtu.getNvrPath());
		map.put("port", dtu.getNvrPort());
		return "success";
	}
}
