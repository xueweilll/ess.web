...
import com.hibernate.entity.Alarm;
...
public class HistoryPoliceAction extends ActionBase {
	...
	@Autowired
	private AlarmMsgService sensorService;
	...
	public String historyPoliceDate() {
		...
		try {
			...
			Object[] obj = null;
			list=sensorService.getAlarmMsg(hql, obj);
		...
		try {
			...
			hisIrList = sensorService.listAlarmMsg(start, limit,hql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
